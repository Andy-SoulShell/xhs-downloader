...
export function PublicationVerificationResume({ onResume }: PublicationVerificationResumeProps) {
  ...
  const resume = async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "验证恢复请求失败，请重试");
    } finally {
  ...
  return (
    <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
      <div className="flex items-start gap-2">
        <ScanLine aria-hidden className="mt-0.5 shrink-0 text-amber-700" size={16} />
      ...
      {accepted ? (
        ...
        >
          ...
          {error && (
            <p aria-live="assertive" className="mt-2 text-xs leading-5 text-red-700" role="alert">
              继续失败：{error}
            </p>
          )}
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <ActionButton disabled={submitting} onClick={() => void resume()}>
