...
export function ConnectionPanel({ account, browserDriver, managedBrowser }: ConnectionPanelProps) {
  ...
  return (
    <>
      ...
      />
      <ManagedBrowserPanel control={managedBrowser} selected={browserDriver === "managed"} />
      <section aria-label="连接状态" className="mt-6">
        <div className="grid gap-3 md:grid-cols-3">
          <StatusCard
            description={browserDriver ? connectionCopy[browserDriver].label : "还没有选择连接方式"}
            icon={MonitorCheck}
            label="连接方式"
          ...
          />
          <StatusCard
            description={latestExtension ? "浏览器插件正在正常通信" : "等待插件首次连接"}
            icon={Activity}
            label="最近通信"
            tone={latestExtension?.online ? "success" : "neutral"}
            value={latestExtension ? formatRelativeTime(latestExtension.last_seen_at) : "尚未连接"}
          />
          <StatusCard
            description={
              account?.logged_in
        ...
        <section aria-label="已连接的插件" className="mt-8">
          <div className="mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-stone-950">已连接的插件</h2>
    ...
    <article className="control-shell min-w-0 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-sm text-stone-700">浏览器插件</p>
          <p className="meta-text mt-1">最近通信 {formatRelativeTime(extension.last_seen_at)}</p>
        </div>
    ...
    <article className="control-shell min-w-0 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="meta-text mt-1 truncate">{description}</p>
        </div>
      </div>
    </article>
  );
