...
describe("软件自带浏览器控制区", () => {
  ...
  it("运行时允许停止和刷新并显示服务端消息", () => {
    ...
    expect(screen.getByText("可以选用")).toBeInTheDocument();
    expect(screen.getByText("专用浏览器已启动，登录状态将保存在本机")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "停止" }));
    fireEvent.click(screen.getByRole("button", { name: "刷新软件自带浏览器状态" }));
  ...
  it.each([
    [
      "软件自带浏览器用户目录已由另一个服务实例占用",
      "请关闭另一个正在使用此软件自带浏览器目录的 xhs-downloader 服务",
    ],
  ...
  it("操作期间显示过渡状态并禁用全部控制", () => {
    ...
    expect(screen.getByRole("button", { name: "刷新软件自带浏览器状态" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "停止" })).toBeDisabled();
  });
});
