...
describe("受管浏览器控制区", () => {
  ...
  it("从停止状态启动并展示首次登录引导", () => {
    ...
    render(
      <ManagedBrowserPanel
        control={makeManagedBrowserControl({ start })}
        selected
      />,
    );
  ...
  it("运行时允许停止和刷新并显示服务端消息", () => {
    ...
    expect(
      screen.getByText("专用浏览器已启动，登录状态将保存在本机"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "停止" }));
    fireEvent.click(
      screen.getByRole("button", { name: "刷新受管浏览器状态" }),
    );
  ...
  it("未安装时禁用启动并给出配置路径", () => {
    ...
    expect(
      screen.getByText(/请安装 Chrome 或 Chromium/),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "启动" })).toBeDisabled();
  });
  ...
  it.each([
    ...
    [
      "专用浏览器启动失败",
      "请先刷新状态；若仍失败，请检查浏览器可执行文件设置",
    ],
  ])("把操作错误转换为具体修复提示", (error, repair) => {
    render(
      <ManagedBrowserPanel
        control={makeManagedBrowserControl({ error })}
        selected
      />,
    );
  ...
  it("操作期间显示过渡状态并禁用全部控制", () => {
    render(
      <ManagedBrowserPanel
        control={makeManagedBrowserControl({ operation: "start" })}
        selected
      />,
    );
