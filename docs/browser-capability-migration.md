...
- 扩展已为收藏和发布提供任务级授权的可信输入；评论与回复仍需迁移并补全
  ...
  写入实现，扫码登录、浏览器与桌面服务重启后的会话恢复、点赞、收藏、私密视频
  和官方定时图文均已完成真实验收。
...
- Cookie、代理和路由字段会排空当前请求后原子热替换；写任务在提交时冻结所选
  浏览器驱动，禁止跨模式自动重试。
- HTTP 统一读取 Provider 已真实支持推荐与详情；Cookie 和 User-Agent 必须来自
  同一请求。搜索或主页的静态页面没有可靠结果时返回结构化失败，由混合路由在
  同账号门禁通过后回退浏览器，不把空搜索或二级页面访客态伪装成成功，也不扩展
  到写操作。
- 扩展五项读取、受管 Profile 的推荐/搜索/详情/指定与当前主页，以及未配置
  Cookie 时 HTTP→受管浏览器安全回退已真实通过；最终桌面包中，有效同会话
  Cookie + User-Agent 的 `http_only` 推荐与详情真实成功，`http_first` 搜索与
  两种主页均取得 `matched` 后安全回退受管浏览器。
- 受管 Chromium 被强制终止后能准确报告异常，重新启动会取得新的回环 CDP 端口并
  恢复真实读取，不影响用户日常 Chrome。
- 登录适配器不再把孤立头像或缺少明确非访客状态的过渡页面当作登录成功；受管
  浏览器对启动阶段的未登录快照做有限稳定等待。最终桌面包扫码后，浏览器与桌面
  服务分别重启仍保持登录，启动后立即检查也取得严格账号凭证。
...
- 受管私密图文、视频和平台定时发布已接入共享 Chromium、独占执行闸门及 API
  ...
  同一任务且不会重复激活发布。封闭发布按钮不再依赖坐标：执行器从 CDP
  无障碍树唯一确认并聚焦“发布/定时发布”语义节点，再发送可信 Space。私密视频
  与官方定时图文已真实通过；首条图文因用户可能同时点击而保留部分通过结论。
...
- WebUI 将 Cookie + HTTP 标为高级可选模式，默认收起安全获取说明，只指导用户从
  同账号请求中复制 Cookie 与 User-Agent 值，不引导复制 Set-Cookie、整条 cURL、
  其他请求头或把凭据发到聊天和日志；Cookie 输入仍为密码类型，保存后不回显。
- 混合路由已接入一次性账号一致性门禁：HTTP 在 Provider 内计算 HMAC，协议 5
  ...
  最终桌面包的有效 HTTP 与受管会话已经取得 `matched` 并安全回退；真实
  `different` 仍待具备第二个授权账号时复验。
