...
| 关键词搜索 | API、MCP、WebUI | 已完成 | 支持基础搜索；非默认筛选当前明确拒绝 |
| 搜索筛选 | API、MCP | 待增强 | 模拟筛选面板交互并读取更新后的页面状态 |
| 帖子详情 | API、MCP、WebUI | 已完成 | 返回正文、媒体、互动数据和已加载评论 |
| 全量评论 | API、MCP | 待增强 | 有界滚动、回复展开、数量上限和超时 |
| 指定用户主页 | API、MCP | 已完成 | 解析资料、统计项和帖子摘要 |
| 当前账号主页 | API、MCP | 已完成 | 通过侧边栏站内导航后解析 |
| 点赞/取消点赞 | API、MCP | 待迁移 | 目标状态语义，操作后重新读取并核验 |
| 收藏/取消收藏 | API、MCP | 待迁移 | 目标状态语义，操作后重新读取并核验 |
| 发表评论 | API、MCP | 待迁移 | 幂等请求、提交后查找新评论并核验 |
| 回复评论 | API、MCP | 待迁移 | 评论 ID 或用户 ID 定位，提交后核验 |
...
1. 增加主世界状态桥接和搜索筛选交互，消除基础搜索限制。
2. 增加有界评论滚动与“更多回复”展开，保留数量和时间上限。
3. 实现点赞、收藏的目标状态操作与操作后核验。
4. 实现评论、回复的提交后核验及 `needs_review` 分支。
5. 为写能力增加 WebUI 二次确认、审计记录和 MCP 写操作注解。
6. 使用真实登录浏览器进行人工验收，但不把真实内容或凭据写入仓库。
