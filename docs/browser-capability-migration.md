...
| 删除 Cookie | API、MCP、WebUI、扩展 | 已完成 | 浏览器按站点清理；HTTP 模式清除配置并热替换客户端 |
...
- 扩展已为收藏和发布提供任务级授权的可信输入；评论与回复仍需迁移并补全
  安全验证后的原任务恢复。`ManagedBrowserProvider` 仍作为无扩展安装场景的
  后续写入实现，而不是当前扩展可信输入的前置条件。
...
- 读取接口已支持 `http_only`、`browser_only`、`http_first` 和
  `browser_first`，成功响应包含实际来源、浏览器驱动与安全回退原因。
- Cookie、代理和路由字段会排空当前请求后原子热替换；写任务在提交时冻结所选
  浏览器驱动，禁止跨模式自动重试。
- 扩展的五项读取与 HTTP→扩展回退已真实通过；受管匿名 Profile 的推荐、详情和
  指定主页已通过，扫码后的搜索、当前账号主页及写操作仍待现场确认。
- 受管点赞和收藏已实现目标状态预检、可信点击、严格回读和
  `failed/needs_review` 分界；受管发布与验证暂停恢复仍在实施。
...
- `platform_scheduled`：由扩展按 `Asia/Shanghai` 填写创作页，通过可信输入
  把绝对时间交给小红书官方排期；验收必须回读平台的非零
  `schedule_post_time`。
