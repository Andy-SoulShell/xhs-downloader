...
| 能力路径 | 当前已有能力 | 当前边界 |
| --- | --- | --- |
| Cookie + HTTP | 短链接解析、作品详情、初始状态解析、媒体下载；统一读取 Provider 已支持推荐、默认筛选搜索、详情、指定主页及严格身份确认后的当前主页 | Cookie 必须由用户显式配置；非默认搜索筛选返回 `unsupported`，仅在允许回退的策略下转交浏览器；不承担互动和发布；当前配置已清除，成功读取路径待重新配置后复验 |
| 浏览器扩展 | 扫码登录、推荐、搜索、详情、评论、主页、互动、发布、站点 Cookie 清理 | 已接入统一路由并真实通过；评论仍受平台安全验证限制 |
| 受管浏览器 | 专用 Chromium、独立 Profile、登录、推荐、搜索、详情、主页、点赞、收藏、私密图文/视频及平台定时发布 | 匿名读取、异常退出检测和恢复已真实通过；扫码确认、登录持久化及写操作仍待独立 Profile 现场验收 |
...
- HTTP 统一读取 Provider 的推荐、默认筛选搜索、详情、指定主页和严格身份确认后的
  ...
  `unsupported`。
- 当前未配置 Cookie 时，`http_only` 结构化返回 `not_configured`，
  `http_first` 已真实安全回退到受管浏览器；配置真实 Cookie 后的统一读取成功路径
  仍需重新验收。
- 受管浏览器在匿名独立 Profile 中读取推荐、详情和指定主页成功；匿名搜索
  合法返回空列表。专用进程被强制终止后能准确报告异常，并以新回环 CDP 端口恢复。
  独立 Profile 的扫码确认、登录持久化和账号相关能力仍待现场确认。
- 受管点赞、收藏已完成预检、Playwright 可信点击和同页回读的自动化链路；
  点击后的任何不确定性都会进入 `needs_review`，不会跨驱动重试。
- 受管私密图文、视频、平台定时发布及安全验证恢复已接入生产 Worker，并覆盖
  启动回滚、页面聚焦、暂停恢复和防重复发布测试；真实发布必须在扫码登录后由用户
  再次确认。
...
账号相关的自动回退不应把稳定用户 ID 或昵称写入路由审计。后续门禁采用一次性
账号相等性挑战：HTTP 和当前浏览器只在内存中比较随机挑战值，对外仅返回
`matched`、`different`、`logged_out` 或 `unverified`。

参与当前路由的 Provider 账号不一致时：
...
上述自动一致性门禁尚未接入当前路由器；现有证据只是扩展与 HTTP 会话的一次人工
脱敏比对，也不覆盖受管 Profile。在门禁实现前，推荐流和当前账号主页等个性化读取
若不能人工确认账号一致，应使用 `browser_only` 或 `http_only`，不要启用混合回退。
