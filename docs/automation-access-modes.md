...
| 能力路径      | 当前已有能力                                                                                                                                       | 当前边界                                                                                                                                                                                                     |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Cookie + HTTP | 短链接解析、作品详情、初始状态解析、媒体下载；统一读取 Provider 已真实支持推荐与详情，搜索和主页在静态页面不可靠时返回结构化失败并交由混合路由处理 | Cookie 与同一请求的 User-Agent 必须由用户显式配置；账号证明只在 Provider 内生成一次性 HMAC；不承担互动和发布                                                                                                 |
| 浏览器扩展    | 扫码登录、推荐、搜索、详情、评论、主页、互动、发布、站点 Cookie 清理                                                                               | 已接入统一路由；协议 5 通过独立内存通道和页面 WebCrypto 生成账号证明，不写浏览器任务或存储；评论仍受平台安全验证限制                                                                                         |
| 受管浏览器    | 专用 Chromium、独立 Profile、登录、推荐、搜索、详情、主页、点赞、收藏、私密图文/视频及平台定时发布                                                 | 账号证明通过同一控制器、独占闸门和 CDP 会话在页面内计算；扫码、浏览器与桌面服务重启后的登录持久化、异常恢复、真实读取、互动及私密视频/平台定时发布已通过；首条图文触发来源因用户可能同时点击而仍记为部分通过 |
...
路由时才从同账号网页请求中复制 Cookie 与 User-Agent 值，不复制整条 cURL、
Set-Cookie 或其他标头。

最终桌面包已使用同一受管 Profile 的 Cookie 与 User-Agent 验证有效 HTTP 会话；
`http_only` 推荐和详情真实成功，`http_first` 的搜索、指定主页和当前主页在 HTTP
静态页面不可靠时均通过一次性账号门禁取得 `matched`，再安全回退受管浏览器。
Cookie 不能替代帖子链接携带的 `xsec_token` 等访问上下文；私密帖子去掉该上下文后
仍可能没有可解析的作品数据。
...
- 新版发布控件使用封闭影子根，页面脚本 `.click()`、CDP 回车和宿主元素中心点击
  均不能稳定提交；宿主还同时包含“暂存离开”和“发布”，不能把宿主中心当作
  发布按钮。最终受管路径不再使用坐标：桥接层聚焦内部真实按钮，执行器从 CDP
  无障碍树严格确认唯一的“发布”或“定时发布”按钮，再发送浏览器级可信 Space。
  私密视频由平台立即确认成功；私密官方定时图文进入 `published` 后，笔记管理
  回读到准确的目标标题、定时状态和北京时间。
...
- 扩展源码构建出共享页面适配器，扩展和受管浏览器复用登录、推荐、搜索、
  详情、主页解析与脱敏语义锚点。
- 登录判断只接受明确的非访客账号状态或同源 HTTPS 账号主页锚点；受管浏览器刚
  启动时会对未登录快照进行最长约 5 秒的有限等待，避免页面水合前的短暂假阴性。
...
- HTTP 统一读取 Provider 的推荐、搜索、详情、指定主页和当前主页均有自动化契约
  覆盖；真实静态页面当前可靠承载推荐与详情，搜索和主页在结果不可靠时返回
  类型化失败，由混合路由处理。
...
- 未配置 Cookie 时，`http_only` 会结构化返回 `not_configured`；Cookie 与
  User-Agent 不匹配时会真实返回 `authentication_expired`。匹配同一受管会话后，
  `http_only` 推荐与详情真实成功；HTTP 搜索页没有静态结果时返回 `unsupported`，
  不会把空列表伪装成有效搜索。
- 受管浏览器的独立 Profile 已在扫码后连续取得严格账号凭证；浏览器重启和桌面
  服务完整重启后均恢复登录，启动后立即查询也通过稳定等待返回正确结果。推荐、
  普通搜索、详情和当前主页均真实返回，页面内一次性账号证明为 `proved`。专用
  进程被强制终止后能准确报告异常，并以新回环 CDP 端口恢复。
...
- 受管私密图文、视频、平台定时发布及安全验证恢复已接入生产 Worker，并覆盖
  启动回滚、语义按钮聚焦、暂停恢复和防重复发布测试；私密视频与官方定时图文
  已真实通过，首条图文因用户可能同时点击而保留触发来源不唯一的验收备注。
...
满足严格同源主页证明条件，受管真实页面已返回 `proved`。最终桌面包中，有效
HTTP 与受管会话的 `matched` 路径已真实通过；`different` 路径仍待具备第二个
授权账号时复验。
...
3. 已复用浏览器共享页面适配器；Page Fetch 仍作为浏览器 Provider 内部的可选优化，
   不改变对外能力契约。
4. 已完成专用用户目录、回环 CDP、单实例锁、生命周期、扫码持久化、可信输入和
   异常恢复；登录后的点赞、收藏、私密视频与官方定时图文已完成真实验收。
5. 已完成受管发布安全验证的页面保留、用户接管和同任务恢复；真实风控触发路径待
   现场验收。
6. 已实现 HTTP 推荐、搜索、详情、指定主页和当前主页契约；推荐与详情真实直出，
   搜索和主页按静态页面可靠性显式回退，HTTP 继续限定为只读。
7. 已实现混合路由的脱敏账号一致性门禁；协议 5 扩展挑战通道和过期 Cookie 安全
   回退已真实通过；有效 HTTP 与受管会话的 `matched` 路径也已在最终桌面包通过，
   不同账号的真实 `different` 路径仍待具备第二个授权账号时复验。
