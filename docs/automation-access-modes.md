...
| 能力路径 | 当前已有能力 | 当前边界 |
| --- | --- | --- |
| Cookie + HTTP | 短链接解析、作品详情、初始状态解析、媒体下载 | 推荐、搜索和主页会明确返回“不支持”；不承担互动和发布 |
| 浏览器扩展 | 扫码登录、推荐、搜索、详情、评论、主页、互动、发布、站点 Cookie 清理 | 已接入统一路由并真实通过；评论仍受平台安全验证限制 |
| 受管浏览器 | 专用 Chromium、独立 Profile、登录、推荐、搜索、详情、主页、点赞和收藏 | 匿名推荐、详情和指定主页已真实通过；独立账号扫码及写操作真实验收仍在进行 |
...
Provider、浏览器驱动、尝试顺序和结构化回退原因；同一请求始终租用固定配置
快照，不会在运行中混用更新前后的 Cookie 或驱动。

当前真实验收边界：

- 扩展的推荐、搜索、详情、指定主页和当前账号主页均通过统一接口。
- HTTP 优先时，搜索以 `unsupported`、未配置 Cookie 的详情以
  `not_configured` 安全回退到扩展，回退轨迹与数据一同返回。
- 受管浏览器在匿名独立 Profile 中读取推荐、详情和指定主页成功；匿名搜索
  合法返回空列表。独立 Profile 的扫码登录和账号相关能力仍待现场确认。
