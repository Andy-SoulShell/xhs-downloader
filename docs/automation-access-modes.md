...
| 受管浏览器 | 专用 Chromium、独立 Profile、登录、推荐、搜索、详情、主页、点赞、收藏、私密图文/视频及平台定时发布 | 账号证明通过同一控制器、独占闸门和 CDP 会话在页面内计算；登录持久化、异常恢复、真实读取、互动及私密视频/平台定时发布已通过；首条图文触发来源因用户可能同时点击而仍记为部分通过 |
