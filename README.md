...
独立模式的下载结果保存在扩展本地存储中。结果以浏览器回报的真实状态为准：全部文件
落盘才记为完成，任一文件被取消、写入失败或链接失效则整批记为失败并附上原因，浏览器
在下载期间休眠时会在唤醒后补查确认。服务在线时记录会自动同步，离线时保留在本地，
面板提供显式同步入口。后台模式只负责提交持久化任务，任务由本地服务继续执行，
可在 WebUI 中查看和重试。
