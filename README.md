...
2. 在同一 Chromium 浏览器中登录
   [小红书创作中心](https://creator.xiaohongshu.com/publish)。
3. 在 WebUI 的“发布中心”创建草稿，填写内容并添加图片或视频。

发布模式：

- `一键发布`：冻结当前草稿为发布任务，立即打开创作中心；扩展填充素材和正文，
  在点击官方“发布”按钮前把任务标记为“正在发布”。
- `定时发布`：保存带时区的计划时间；到期后扩展领取任务并打开创作中心执行。浏览器
  完全退出时，扩展闹钟无法唤醒浏览器，因此本地服务、浏览器和扩展必须届时运行。

图文草稿支持 1–18 张图片，视频草稿支持一个视频，二者不能混用。提交任务时会冻结
标题、正文、标签、素材顺序、大小和 SHA-256；之后修改草稿不会改变已经排期的内容。

任务使用短期租约避免多个标签页重复执行。尚未点击发布时发生中断，任务可以安全回到
队列；点击发布后若平台结果无法确认，任务进入“需要确认”，不会盲目重试。请先到创作
中心核对实际结果，再在 WebUI 中决定是否重试。平台页面结构变化、登录失效、账号风控
或平台审核都可能要求人工处理。
...
扩展还会在官方创作中心执行已领取的发布任务，并用 `alarms` 权限轮询到期排期。首次
联动时，扩展只向回环地址登记随机能力令牌；服务端仅保存令牌摘要。扩展只申请帖子页、
创作中心、浏览器下载、本地存储、闹钟和本机服务地址所需权限，不读取或保存浏览器
Cookie、密钥和密码。
